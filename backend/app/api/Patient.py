...
from app.crud.Patient import create_paciente, get_pacientes_by_id,update_paciente, delete_paciente, get_paciente_by_id,get_all_pacientes
...
@router.get("/list/{user_id}", response_model=List[PacientResponse])
def get_user_by_list(user_id: int, db: Session = Depends(get_db)):
    return get_pacientes_by_id(db, user_id)
